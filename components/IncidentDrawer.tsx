...
const stageLabel: Record<IncidentSummary['stage'], string> = {
  verify: '智能核验',
  locate: '定位分析',
  contact: '商户触达',
  escalate: '故障升级',
...
export default function IncidentDrawer({ incident, onClose, onStartDemo }: IncidentDrawerProps) {
  ...
  // 非黄金事故用真实字段描述当前 AI 进度，不再硬编码“真实故障”。
  const nonGoldenJudgment = incident.stage === 'verify'
    ? '待核验'
    : incident.confidence >= 90 ? '疑似真实故障' : '待复核';

  return <div className="drawer-layer"><button className="drawer-backdrop" aria-label="关闭事故详情" onClick={onClose} tabIndex={-1}/><aside className="incident-drawer" role="dialog" aria-label="事故详情">
    ...
    <div className="drawer-scroll">
      {isGolden ? (
        ...
        <section className="drawer-section"><div className="drawer-section-title"><div><BrainCircuit size={15}/><span>AI 结构化决策</span></div><small>Evidence Agent v3.4</small></div><div className="decision-card"><div><span>故障判断</span><b>{nonGoldenJudgment}</b></div><div><span>当前阶段</span><b>{stageLabel[incident.stage]}</b></div><div><span>置信度</span><b>{incident.confidence}%</b></div><div><span>证据链</span><b>等待处置演示后生成完整链路</b></div></div><p className="drawer-generic-note">{incident.id} 的最终根因与触达内容由处置演示链路实时生成；此处展示的是当前档案快照。</p></section>
      )}
      <section className="drawer-section"><div className="drawer-section-title"><div><Target size={15}/><span>五阶段 Agent Trace</span></div><small>{isGolden ? '总耗时 8.92s · ¥0.42' : '示例链路 · 与具体演示无关'}</small></div><div className="trace-timeline">{traces.map((trace,index)=><article key={trace[0]}><span className="trace-index">{index+1}</span><div className="trace-body"><div><strong>{trace[0]}</strong><span className="trace-success"><CheckCircle2 size={12}/>完成</span></div><dl><div><dt>耗时</dt><dd>{trace[1]}</dd></div><div><dt>Token</dt><dd>{trace[2]}</dd></div><div><dt>成本</dt><dd>{trace[3]}</dd></div><div><dt>置信度</dt><dd>{trace[4]}</dd></div></dl><p>{trace[5]}</p></div></article>)}</div></section>
      {isGolden ? (
